<template>
  ...
  const exportToCSV = () => {
  try {
    let csvContent = '订单明细号,采购清单号,商品编号,采购数量,采购单价,采购总价,备注\n';
    searchResults.value.forEach(detail => {
      csvContent += `${detail.id},${detail.list_id},${detail.product_id},${detail.quantity},${detail.price},${detail.total_price},${detail.note}\n`;
    });
