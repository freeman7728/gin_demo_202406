...
```
docker build --build-arg SERVICE=employee_service -t employee_service:latest .
```

```
docker run -d --name employee -p 8084:8084 employee

docker run -d --name employee_Test -p 8081:8080 employee:latest
```
