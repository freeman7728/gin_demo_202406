...
RPC（Remote Procedure Call，远程过程调用）是一种计算机通信协议，允许程序调用另一台计算机上的子程序或服务，就像调用本地子程序一样。RPC 使得分布式计算更为简单和透明，隐藏了网络通信的复杂性。RPC 的基本原理是客户端向服务器发送一个请求，请求服务器执行某个方法，并返回执行结果。RPC 框架通常负责序列化和反序列化数据、网络通信以及错误处理。

### go-micro微服务框架

Go-Micro 是一个功能强大的微服务框架，旨在简化分布式系统的开发。它提供了许多内置功能，包括服务发现、负载均衡、同步通信、异步通信等，可以帮助开发者快速构建和管理微服务架构。它可以自动发现服务，无需硬编码服务地址，支持通过 RPC 进行同步服务调用，支持通过消息队列进行异步服务调用，请求在服务实例之间自动分配，确保负载均衡。

### swagger在线接口文档

Swagger 是一个强大的工具，用于生成、描述、使用和可视化 RESTful Web 服务。使用 Swagger 可以自动生成 API 文档，并提供一个交互式的用户界面，让开发者可以直接在浏览器中测试 API。
...
etcd 是一个强一致性、高可用的分布式键值存储系统，广泛应用于分布式系统中的配置管理和服务发现。通过简单的 API 和高效的存储结构，etcd 提供了可靠的数据存储和监控功能，是现代分布式系统的重要基础组件。
...
Nginx 作为一个高性能、轻量级的反向代理服务器，具有强大的负载均衡、缓存加速、安全控制和灵活配置等特点，适用于各种规模的 Web 应用和服务架构。它不仅提升了系统的性能和稳定性，还能有效地保护和管理网络资源，是现代互联网架构中不可或缺的关键组件之一。
...
Vite 是一个基于现代浏览器原生 ES 模块导入的开发服务器，用于快速开发 Vue.js 单页面应用（SPA）和库。Vite 利用浏览器的原生 ES 模块导入特性，避免了传统打包工具（如Webpack）的繁重的初始构建过程。Vite 支持基于浏览器原生模块的热模块更新（HMR），在开发过程中可以实时更新修改的内容，无需完全刷新页面。

### element-plus

Element Plus 是一套基于 Vue 3 的 UI 组件库，它是对 Element UI 的升级和重构。Element Plus 是基于 Vue 3 构建的，充分利用了 Vue 3 的新特性，如 Composition API，提升了性能和开发效率。Element Plus 提供了模块化的方式引入组件，支持按需加载，减少了整体打包体积，同时提升了应用的加载速度。Element Plus 是一个现代化的 Vue 3 UI 组件库，继承了 Element UI 的优点，并在性能、设计和开发体验上进行了进一步的优化和改进。
...
Node.js 是一个基于 Chrome V8 引擎的 JavaScript 运行时环境，用于构建快速、可扩展的网络应用程序。Node.js 采用单线程模型，但通过事件循环（Event Loop）和异步非阻塞 I/O 操作，可以处理大量并发连接，避免了传统多线程模型中线程切换的开销。Node.js 使用 npm（Node Package Manager）作为其包管理器，拥有丰富的开源模块库和工具，开发者可以轻松地安装、管理和分享代码包。
...
![image-20240623215458329](C:/Users/Administrator/AppData/Roaming/Typora/typora-user-images/image-20240623215458329.png)

该界面可以使用用户修改，注销账户，验证邮箱功能

#### 员工管理页面

![image-20240623220221230](C:/Users/Administrator/AppData/Roaming/Typora/typora-user-images/image-20240623220221230.png)

该界面实现了用户的增删改查，以及csv文件的导入导出

#### 供应商管理页面

![image-20240623215514583](C:/Users/Administrator/AppData/Roaming/Typora/typora-user-images/image-20240623215514583.png)

该界面实现了供应商的增删改查，以及csv文件的导入导出

#### 商品管理页面

![image-20240623215524254](C:/Users/Administrator/AppData/Roaming/Typora/typora-user-images/image-20240623215524254.png)

该界面实现了商品的增删改查，以及csv文件的导入导出

#### 采购管理页面

![image-20240623215534332](C:/Users/Administrator/AppData/Roaming/Typora/typora-user-images/image-20240623215534332.png)

该界面实现了采购清单的增删改查，以及csv文件的导入导出

#### 订单详情管理页面

![image-20240623215545973](C:/Users/Administrator/AppData/Roaming/Typora/typora-user-images/image-20240623215545973.png)

该界面实现了采购详情的增删改查，以及csv文件的导入导出
