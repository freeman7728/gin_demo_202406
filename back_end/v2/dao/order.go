...
func SelectOrderByEmployeeId(employee *models.Employee, out *dto.DetailList) (tx *gorm.DB) {
	sql := `SELECT * FROM list WHERE employee_id = ?`
	return global.DB.Raw(sql, employee.ID).Scan(&out.DetailList)
}
